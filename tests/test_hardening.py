...
from heddle.security.sandbox import (
    SandboxManager, SandboxConfig,
    CONTAINER_WRITABLE_PATH, CONTAINER_SECCOMP_PATH,
)
from heddle.security.sandbox_policy import policy_for, TIER_MATRIX
from heddle.runtime.images import ImageRef, load_registry, resolve as resolve_image
from heddle.config.loader import validate_config as _validate_config, ConfigError
...
# ── Hardening flags (ADR-004 §3) ─────────────────────────────────────

def _t2_sandbox_config():
    return SandboxConfig(agent_name="harden-test", pids_limit=128, user="65534:65534")


def test_sandbox_emits_cap_drop_all():
    args = SandboxManager().generate_docker_run_args(_t2_sandbox_config())
    assert "--cap-drop=ALL" in args


def test_sandbox_emits_no_new_privileges():
    args = SandboxManager().generate_docker_run_args(_t2_sandbox_config())
    assert "--security-opt=no-new-privileges:true" in args


def test_sandbox_emits_pids_limit():
    args = SandboxManager().generate_docker_run_args(_t2_sandbox_config())
    assert "--pids-limit=128" in args


def test_sandbox_emits_user_nobody():
    args = SandboxManager().generate_docker_run_args(_t2_sandbox_config())
    assert "--user=65534:65534" in args


def test_sandbox_strict_seccomp_emitted_only_for_strict_profile():
    default_args = SandboxManager().generate_docker_run_args(
        SandboxConfig(agent_name="t2", seccomp_profile="default")
    )
    strict_args = SandboxManager().generate_docker_run_args(
        SandboxConfig(agent_name="t3", seccomp_profile="strict")
    )
    assert not any("seccomp=" in a for a in default_args)
    assert f"--security-opt=seccomp={CONTAINER_SECCOMP_PATH}" in strict_args


def test_sandbox_writable_volume_mounts_at_var_heddle_agent():
    args = SandboxManager().generate_docker_run_args(
        SandboxConfig(agent_name="vol-test", writable_volume="/host/path")
    )
...
def test_sandbox_container_name_is_heddle_prefixed():
    args = SandboxManager().generate_docker_run_args(
...
def test_policy_t1_has_no_writable_volume():
    assert policy_for(1).writable_volume is False


def test_policy_writable_volume_for_higher_tiers():
    assert policy_for(2).writable_volume is True
    assert policy_for(3).writable_volume is True
    assert policy_for(4).writable_volume is True


def test_policy_pids_limit_scales_with_tier():
    assert policy_for(1).pids < policy_for(2).pids < policy_for(3).pids < policy_for(4).pids


def test_policy_seccomp_strict_for_t3_and_t4():
    assert policy_for(1).seccomp == "default"
    assert policy_for(2).seccomp == "default"
    assert policy_for(3).seccomp == "strict"
    assert policy_for(4).seccomp == "strict"


def test_policy_unknown_tier_falls_back_to_t1():
    assert policy_for(99) is policy_for(1)


# ── Image digest pinning (runtime/images.py) ─────────────────────────

def test_image_registry_loads_python_pin():
    reg = load_registry()
    assert "python-3.12-slim" in reg
    ref = reg["python-3.12-slim"]
    assert ref.image == "python:3.12-slim"
    assert ref.digest.startswith("sha256:")
    assert len(ref.digest) == len("sha256:") + 64  # SHA-256 hex


def test_image_resolve_known_returns_digest_pinned():
    ref = resolve_image("python-3.12-slim")
    assert "@sha256:" in ref
    assert ref.split("@", 1)[0] == "python:3.12-slim"


def test_image_resolve_unknown_passes_through():
    assert resolve_image("not-in-registry:latest") == "not-in-registry:latest"


def test_generated_sandbox_uses_digest_pinned_image():
...
""")
    config = validate_config(raw, source="<test>")
    sandbox = SandboxManager().generate_sandbox_config(config)
    assert "@sha256:" in sandbox.image


# ── T1 no-writable-volume + T4 cannot opt out (loader + sandbox) ─────

def test_t1_generated_sandbox_has_no_writable_volume():
    import yaml as _yaml
    from heddle.config.loader import validate_config
...
""")
    config = validate_config(raw, source="<test>")
    sandbox = SandboxManager().generate_sandbox_config(config)
    assert sandbox.writable_volume == ""
    args = SandboxManager().generate_docker_run_args(sandbox)
    assert not any(a.startswith("-v=") for a in args)


def test_t4_cannot_opt_out_of_sandbox():
    import yaml as _yaml

    raw = _yaml.safe_load("""
agent:
  name: privileged-noop
  version: "1.0.0"
  description: "T4"
  model:
    provider: none
  ...
  triggers:
    - type: on_demand
""")
    with pytest.raises(ConfigError, match="cannot opt out of sandboxing"):
        _validate_config(raw, source="<test>")


def test_t4_with_docker_sandbox_passes():
    import yaml as _yaml
...
agent:
  ...
  triggers:
    - type: on_demand
""")
    config = _validate_config(raw, source="<test>")
    assert config.agent.runtime.trust_tier == 4
    assert config.agent.runtime.sandbox == "docker"


def test_t3_with_sandbox_none_still_allowed():
    # Backwards-compat: only T4 is mandatory-sandbox. T3 can still opt out
    # for now; tightening this is a separate decision.
    import yaml as _yaml
...
""")
    config = _validate_config(raw, source="<test>")
    assert config.agent.runtime.sandbox == "none"

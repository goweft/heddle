"""Load and validate Heddle agent YAML configs."""
...
def validate_config(data: dict, source: str = "<unknown>") -> AgentConfig:
    ...
    # T4 (privileged) cannot opt out of sandboxing.
    # MILESTONE-v0.2.md Pillar 1 DoD; ADR-004 §5.
    if (
        config.agent.runtime.trust_tier == 4
        and config.agent.runtime.sandbox == "none"
    ):
        raise ConfigError(
            f"Agent '{config.agent.name}' has trust_tier: 4 (privileged) but "
            f"runtime.sandbox is 'none'. Privileged tier cannot opt out of "
            f"sandboxing; set runtime.sandbox: docker."
        )

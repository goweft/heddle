"""Heddle agent evolution — passive research and self-improvement.

Scans GitHub, HuggingFace, and the MCP ecosystem for:
1. New MCP server patterns that could become Heddle agents
...
Works with the existing weft-evolve system and Heddle's own generator
...
EVOLVE_DIR = Path.home() / ".heddle" / "evolve"
...
class LoomEvolve:
    """Manages the evolution and self-improvement of the Heddle platform."""
    ...
    def analyze_coverage_gaps(self) -> list[dict]:
        """Find weftbox services that don't have Heddle agents yet."""
        gaps = []
        for service, info in WEFTBOX_SERVICES.items():
            if not info["has_agent"]:
                gaps.append({
                    "service": service,
                    "port": info["port"],
                    "suggestion": f"Create a Heddle bridge agent for {service}",
    ...
    def generate_agent_brief(self, service_name: str) -> str:
        ...
        This is what gets passed to `heddle generate` to create a new agent.
        """

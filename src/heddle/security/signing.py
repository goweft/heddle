"""Heddle config signing and agent quarantine.
...
(~/.heddle/quarantine/) and require explicit promotion before they
...
DEFAULT_KEY_FILE = Path.home() / ".heddle" / "signing.key"
DEFAULT_QUARANTINE_DIR = Path.home() / ".heddle" / "quarantine"
SIGNATURES_FILE = Path.home() / ".heddle" / "signatures.json"
...
class ConfigSigner:
    ...
    The signing key is stored in ~/.heddle/signing.key (generated on
    first use). Signatures are stored in ~/.heddle/signatures.json
    ...
    def verify(self, config_path: str | Path) -> bool:
        ...
        if expected is None:
            ...
            raise SignatureError(f"No signature for {path.name}. Sign it first with `heddle sign`.")
